...
const searchSchema = z.object({
	...
	type: z.enum(['title', 'description', 'all']).optional()
});
...
export const GET = async ({ url }) => {
	try {
		...
		if (type === 'title') {
			...
			// Búsqueda en todos los campos (title o description)
			filter = {
				$or: [
		...
		// Buscar tareas en MongoDB
		const results = await Tareas.find(filter).sort({ order: 1 }).lean();
